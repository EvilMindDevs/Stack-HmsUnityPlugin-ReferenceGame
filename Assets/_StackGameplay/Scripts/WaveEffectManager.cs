...
namespace StackGamePlay
{
    public class WaveEffectManager : MonoBehaviour
    {
        ...
        IEnumerator OnEffect(GameObject target, int effectCount)
        {
            ...
            LeanTween.value(1f, 0f, effectDuration).setOnUpdate((value) => { mat.SetFloat("_Opacity", Mathf.Clamp(value * 2, 0f, 1f)); }).setOnComplete(() => { PushToPool(defaultEffect); }).setEaseOutQuad();

            if (effectCount < multipleEffectCondition) yield break;


            if (effectCount <= MechanicManager.Instance.GameData.perfectCondition)
            {
                ...
                for (int i = 1; i <= multiEffectCount; i++)
                {
                    ...
                    LeanTween.value(1f, 0f, leftTime).setOnUpdate((value) =>
                    {
